    ...
    for key, symbol in TICKERS.items():
        try:
            ...
            if not hist_clean.empty and len(hist_clean) >= 2:
                ...
                if key == 'US10Y':
                    # TNX is 10x Yield. (e.g. 42.0 -> 43.0 is +1.0 diff).
                    # 1.0 diff in TNX = 10 bps.
                    # We want Basis Points. Multiplier 10 is correct.
...
def main():
    ...
    t1, t2, t3, t4 = st.tabs(["🚀 Dashboard", "📊 Heatmap", "🌊 Liquidity Flow", "📖 Master Playbook"])
    ...
    with t3:
        ...
        st.info("Visualizes how Fed Policy flows downstream to specific sectors. This is the wiring diagram of the global economy.")
        ...
        with col_expl:
            with st.expander("1. The Source (Fed/Treasury)", expanded=True):
                st.markdown("""
                * **Liquidity:** When Fed buys assets (QE) or Treasury spends cash (TGA down), liquidity rises.
                * **Impact:** Liquidity seeks the "riskiest" home first (Crypto, Tech).
                """)
            with st.expander("2. The Transmission (Rates/USD)", expanded=True):
                st.markdown("""
                * **Real Yields:** If rates rise faster than inflation, it kills Gold and Tech valuations.
                * **Dollar:** A strong DXY acts as a wrecking ball for Emerging Markets and Commodities.
                """)
            with st.expander("3. The Destination (Assets)", expanded=True):
                st.markdown("""
                * **Long Duration:** Tech/Crypto need *low rates* and *high liquidity*.
                * **Real Assets:** Oil/Banks need *high growth* and *inflation*.
                """)
